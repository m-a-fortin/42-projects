...
/*   Updated: 2021/09/16 13:41:03 by mafortin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	sl_map_standards(char **map, t_main *structs)
{
	...
	while (map[index])
	{
		...
		structs->enm += ft_char_search(map[index], 'X');
		index++;
	}
	if (exit != 1 || structs->link->collect_goal == 0 || player != 1)
		return (-1);
	else
		return (1);
}
...
void	sl_valid_map_limit(t_map *map_data, t_main *structs)
{
	...
	{
		if (map_data->line[0][y] != '1'
			|| map_data->line[map_data->x][y] != '1')
			sl_map_invalid(structs);
		y++;
	}
	while (map_data->line[x])
	{
		if (map_data->line[x][0] != '1'
			|| map_data->line[x][map_data->y] != '1')
			sl_map_invalid(structs);
		x++;
	}
}
